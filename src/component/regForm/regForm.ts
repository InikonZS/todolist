...
export class RegForm extends Control {
  ...
  constructor(parentNode:HTMLElement){
    ...
    this.nameInput = new Input(this.field,'Name',(param)=>{return param.match(NAMEVALIDATION) && param.length >= 3 && param.length <= 20? null : 'enter valid name'},'enter your name','text');
    this.passwordInput = new Input(this.field,'Password',(param)=>{return param.match(PASSWORDVALIDATION)? null : 'enter valid password'},'enter your password','text');
    ...
    this.note.innerText = 'Name length Minimum 3  &  Maximum 20 characters Only letters allowed , password Minimum eight characters, at least one uppercase letter, one lowercase letter and one number:';
    this.node.appendChild(this.note)
    this.submitButton.node.addEventListener('click',()=>{
      this.model.registerUser(this.getData())
      this.node.remove();
  ...
  getData():IAuthData{
    const obj: IAuthData = {
      ...
      password: this.nameInput.getValue(),
    };
    return obj;
  }
}
