...
class FieldView extends Control {
  ...
  constructor(parentNode: HTMLElement, initialState:Array<string>) {
    super(parentNode);
    for (let i = 0; i < 8; i++) {
      const row = new Control(this.node, 'div', 'row');
      for (let j = 0; j < 8; j++) {
        const cell = new CellView(row.node, (j % 2 && i % 2) || (!(j % 2) && !(i % 2)) ? 'cell cell__light' : 'cell cell__dark');
        cell.onClick = () => {
          const cellPos = this.getCellPosition(this.selectedCell);
          ...
          if (this.selectedCell && this.selectedCell.figure) {
            const cellPos = this.getCellPosition(this.selectedCell);
            const allowed = this.model.getAllowed(this.model.state, cellPos.x, cellPos.y);
